...
layout: single
permalink: /developer/coding/ides/vscode/setup_mac/
toc: true
toc_sticky: true
---
...
![Step 1. Switch to the source control view (if it's not there go to `"View"->"Source Control"`). (If you have files open you will instead see the source control for those files. To add a repo close the files or skip ahead to step 9).](assets/mac_setup/source_control_view.png)
...
![Step 2. (With git installed) click "Clone Repository".](assets/mac_setup/clone_repo.png)

![Step 3. Click "Clone from Github".](assets/mac_setup/clone_from_github.png)

![Step 4. If asked, allow the GitHub extension to sign in.](assets/mac_setup/allow_github.png)

![Step 5. You should now see all the repos you have. Search for the one you want. For sake of the tutorial search for `NWChemEx/.github`, i.e., this repo.](assets/mac_setup/your_repos.png)

6. That will bring up Finder. Select the workspace directory as the
   destination.

![Step 7. When prompted open the repository.](assets/mac_setup/open_cloned_repo.png)

![Step 8. If prompted, decide whether you trust us (we'll assume you do and click yes).](assets/mac_setup/trust_us.png)

![Step 9. Once there are files in your workspace the souce control view will change to allow you to interact with the version control of the open files. Unfortunately, this means we can not add additional repositories the same way we did in Step 2. Rather, you need to use the command palate (command + shift + P) then search `Git: Clone`. That will return you to Step 3 above, with the caveat that what was Step 7 should be replaced by Step 10 below.](assets/mac_setup/search_git_clone.png)

![Step 10. Since you have files in your workspace already the dialog box from Step 7 will change and you will want to select "Add to Workspace" instead of "Open".](assets/mac_setup/add_to_workspace.png)

![Step 11. Repeat Steps 9 and 10 until you have all the repositories you want. Once you are done save your workspace by going to `File->"Save Workspace As..."`. We recommend saving it into the `nwchemex` workspace directory and naming it as `nwchemex.code-workspace`.](assets/mac_setup/save_workspace.png)
...
![Step 1. Click on `No active kit`. If you had previously selected a kit, this may instead display the name of that kit.](assets/mac_setup/click_no_active_kit.png)

![Step 2. Select `Clang <version goes here>`.](assets/mac_setup/select_clang.png)
...
![Step 3.  If you want to always use the same options with the same tool kit (e.g., always use Ninja generator), you can edit the tool kit file. To do this, open the command palette (command + shift + P) and search for  `Edit User-Local CMake Kits`.](assets/mac_setup/edit_kit.png)

![Step 4. If we want to always use the same toolchain file with the same toolkit, we can edit the kit to know about the toolchain file. Add `"toolchainFile": "<path/to/the/toolchain>"` to your kit.](assets/mac_setup/add_toolchain_to_kit.png)
...
![Step 1. If you have multiple git projects open, make sure you select the one you want to configure.](assets/mac_setup/select_active_project.png)

![Step 2. Click `CMake: [Debug]: Ready` to start configuring.](assets/mac_setup/run_configure.png)

![Step 3. Select the configuration you would like.](assets/mac_setup/select_configuration.png)
...
![Step 1. Set the project, build configuration, and kit to what you want.](assets/mac_setup/select_what_to_build.png)

![Step 2. Click build.](assets/mac_setup/click_build.png)

![Step 3. Assuming Step 2 is successful, click on "Run CTest" to run the test suite.](assets/mac_setup/run_ctest.png)
...
![Step 1. Switch to the source control view.](assets/mac_setup/switch_to_source_control.png)

![Step 2. (Optional) select the file(s) you want to stage by clicking on the "+" next to the file(s). If you do not stage files VSCode will assume you want to commit all changed files.](assets/mac_setup/stage_changes.png)

![Step 3. Type a descriptive commit message.](assets/mac_setup/commit_message.png)

![Step 4. Click on the commit button. At this point your changes are only committed to your local copy of the repository. We still need to push them to GitHub.](assets/mac_setup/click_commit.png)

![Step 5. Once you click "Commit" the button should change to "Synch Changes". Click that to push your changes to GitHub.](assets/mac_setup/click_synch.png)
