---
title: "Setting Up VSCode on MacOS"
layout: single
permalink: /developer/ides/vscode/setup_mac/
toc: true
toc_sticky: true
---
...
[website](https://code.visualstudio.com/Download). Select the version you
...
![First you will be asked to pick your theme. The choice is up to you.](assets/mac_setup/pick_theme.png)
...
![You will be prompted to choose the coding languages you will use.](assets/mac_setup/choose_apps.png)

![The VSCode C++ extension.](assets/mac_setup/cxx_extension.png)

![Install the Python extension.](assets/mac_setup/python_extension.png)

![If necessary, install command line developer tools to get access to Python.](assets/mac_setup/install_command_line_tools.png)
...
![Tune VSCode's settings to your liking.](assets/mac_setup/tune_settings.png)

![Recommended to turn on settings synch to ensure a consistent VSCode experience across devices.](assets/mac_setup/synch_settings.png)
...
![VSCode has a lot of settings. If you're new to IDEs you probably just want to start with the suggestions below.](assets/mac_setup/settings_open.png)
...
`"View"->"Source Control"`). (If you have files open you will instead see the 
source control for those files. To add a repo close the files or skip ahead to 
step 9).
- If you have not previously installed git see 
  [here](/community/dependencies/dependencies.md) (if you click on "Download Git
  for macOS", it'll just redirect you to git's website which will duplicate our
  instructions). Once git is installed, click `reload` in the source control
  view.

![VSCode Source Control View.](assets/mac_setup/source_control_view.png)

**Step 2.** (With git installed) click "Clone Repository".

![Clone a repository with VSCode.](assets/mac_setup/clone_repo.png)

**Step 3.** Click "Clone from Github".

![Cloning with git](assets/mac_setup/clone_from_github.png)

**Step 4.** If asked, allow the GitHub extension to sign in.

![Allow the GitHub extension to sign in.](assets/mac_setup/allow_github.png)

**Step 5.** You should now see all the repos you have. Search for the one you
want. For sake of the tutorial search for `NWChemEx/.github`, i.e., this repo.

![Searching the file explorer for a repository.](assets/mac_setup/your_repos.png)

**Step 6.** That will bring up Finder. Select the workspace directory as the
   destination.

**Step 7.** When prompted open the repository.

![Opening the repository.](assets/mac_setup/open_cloned_repo.png)

**Step 8.** If prompted, decide whether you trust us (we'll assume you do and
click yes).

![Agreeing to trust the newly opened repo.](assets/mac_setup/trust_us.png)
...
Unfortunately, this means we can not add additional repositories the same way we
did in Step 2. Rather, you need to use the command palate (command + shift + P)
then search `Git: Clone`. That will return you to Step 3 above, with the caveat
that what was Step 7 should be replaced by Step 10 below.

![Using the command palete to clone additional repos.](assets/mac_setup/search_git_clone.png)
...
![Add the new repo to the workspace.](assets/mac_setup/add_to_workspace.png)
...
`File->"Save Workspace As..."`. We recommend saving it into the `nwchemex` 
workspace directory and naming it as `nwchemex.code-workspace`.

![Saving the workspace to a file once fully constructed.](assets/mac_setup/save_workspace.png)
...
**Step 1.** Click on `No active kit`. If you had previously selected a kit, this
may instead display the name of that kit.

![Clicking "no active kit."](assets/mac_setup/click_no_active_kit.png)

**Step 2.** Select `Clang <version goes here>`.

![Selecting the clang version.](assets/mac_setup/select_clang.png)
...
**Step 3.**  If you want to always use the same options with the same tool kit
(e.g., always use Ninja generator), you can edit the tool kit file. To do this,
open the command palette (command + shift + P) and search for 
`Edit User-Local CMake Kits`.

![Editing the kit.](assets/mac_setup/edit_kit.png)

**Step 4.** If we want to always use the same toolchain file with the same
toolkit, we can edit the kit to know about the toolchain file. Add 
`"toolchainFile": "<path/to/the/toolchain>"` to your kit.

![Adding a toolchain to the kit.](assets/mac_setup/add_toolchain_to_kit.png)
...
![Selecting a project.](assets/mac_setup/select_active_project.png)

**Step 2.** Click `CMake: [Debug]: Ready` to start configuring.

![Configuring the CMake build.](assets/mac_setup/run_configure.png)

**Step 3.** Select the configuration you would like.

![Selecting CMake configuration.](assets/mac_setup/select_configuration.png)

#### Troubleshooting

- If you get an error about the CMake executable like below, it most likely
  means you do not have CMake installed (see obtaining_dependencies_on_macos for
  instructions) or VSCode can not find CMake. To set the CMake executable open
  the command pallette (command + shift + P) and search for `cmake.cmakePath`.

![Bad CMake executable error.](assets/mac_setup/bad_cmake_executable.png)

- If you get an error about `NWX_MODULE_DIRECTORY` not being set, it either
  means you did not pass your toolchain to CMake (see
  creating_a_cmake_toolchain_file) or you did not set `NWX_MODULE_DIRECTORY` in
  your toolchain file.

![Failure to set `NWX_MODULE_DIRECTORY`.](assets/mac_setup/nwx_module_dir_not_set.png)

- If the configuration fails because of a missing dependency consult 
  [this page](/community/dependencies/dependencies.md).

![CMake could not locate the Python header files.](assets/mac_setup/missing_python_headers.png)
...
![Select what to build](assets/mac_setup/select_what_to_build.png)

**Step 2.** Click build.

![Build project](assets/mac_setup/click_build.png)

**Step 3.** Assuming Step 2 is successful, click on "Run CTest" to run the test
suite.

![Run CTest](assets/mac_setup/run_ctest.png)
...
![Source control view.](assets/mac_setup/switch_to_source_control.png)

**Step 2.** (Optional) select the file(s) you want to stage by clicking on the
"+" next to the file(s). If you do not stage files VSCode will assume you want
to commit all changed files.

![Stage changes.](assets/mac_setup/stage_changes.png)

**Step 3.** Type a descriptive commit message.

![Add commit message.](assets/mac_setup/commit_message.png)

**Step 4.** Click on the commit button. At this point your changes are only
committed to your local copy of the repository. We still need to push them to
GitHub.

![Commit changes.](assets/mac_setup/click_commit.png)

**Step 5.** Once you click "Commit" the button should change to "Synch Changes".
Click that to push your changes to GitHub.

![Synch changes.](assets/mac_setup/click_synch.png)

#### Troubleshooting

If you get an error about `user.name` and/or `user.email` for git is not set.
i.e., something like:

![Error when your git credentials have not been set.](assets/mac_setup/error_git_name_not_set.png)
...
![How to set the email and user name for git.](assets/mac_setup/set_git_name.png)

When attempting to synchronize with GitHub, if you get a cryptic error like:

![Cryptic error when synchronizing.](assets/mac_setup/cryptic_error.png)
