---
title: "Design of the Python Binding Architecture"
layout: single
permalink: /author/architecture/faqs/py_bindings/
toc: true
toc_sticky: true
---

Section why_is_the_nwchemex_api_written_in_python discusses why we want
...
why_is_the_nwchemex_api_written_in_python, NWChemEx's high-level API is
...
Full featured
   ...
   - Combined with pb_diff_lang we must also acknowledge that at the
     ...
     pb_features will allow users to resume using the full feature set
...
Minimally invasive
   ...
   - As a corollary, it is fine if the tool internally uses an intermediate
     representation; pb_minimally_invasive is really focused on avoiding
...
overwhelming seems to prefer pybind11 over Boost.Python, but Boost.Python
...
- Performance. pybind11 (an admittedly very biased source) seems to
...
- Cppyy seems to acknowledge the packaging problems (see
  [here](https://cppyy.readthedocs.io/en/latest/cmake_interface.html)) and
...
considerations raised in pb_considerations. Notably, the CMake
infrastructure did not satisfy  pb_native_apis. In practice, Cppyy
suggests two mechanisms for  addressing pb_native_apis: pythonizations
...
![Current status of pybind11 Python bindings for SimDE.](assets/simde_exposure.png)

`fig_simde_exposure` summarizes the binding status of SimDE as of this
...
![Summary of how plugins are exposed to Python/C++.](assets/plugin_exposure.png)

`fig_plugin_exposure` shows the proposed mechanism for exposing plugins
...
![Summary of user API interactions with Python bindings.](assets/user_api.png)
...
The core of NWChemEx interacts with the UI according to fig_user_api.
Here the UI is simply labeled as driver, but it need not actually be a driver
in the classical sense. As fig_user_api shows the UI is envisioned as
...
- [This](https://realpython.com/python-bindings-overview/) tutorial covers
  some of the other options available in more detail and was used to partially
  populate the list in pb_existing_choices.
